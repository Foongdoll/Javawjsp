...
-- 댓글 테이블(boardReply)에서 board테이블의 고유번호 35번글에 딸려있는 댓글의 개수는?
select count(*) from boardReply where boardIdx = 35;

select boardIdx,count(*) from boardReply where boardIdx = 1;
-- 댓글 테이블(boardReply)에서 board테이블의 고유번호 35번글에 딸려있는 댓글의 개수는?
-- 원본글의 고유번호와 함께 출력, 갯수의 별명은 replyCnt
-- 원본글을 쓴 닉네임을 함께 출력하시오, 단 닉네임은 board(원본글) 테이블에서 가져와서 출력하시오.

select nickName,boardIdx,count(*) AS replyCnt from boardReply where boardIdx = 1;

-- board 테이블에서 nickName where idx = 1 을 가져옴 
select (SELECT nickName from board where idx = 1) AS nickName,
boardIdx,count(*) 
from boardReply where boardIdx = 1;

-- 앞의 문장을 부모 테이블(board)의 관점에서 보자
SELECT mid, nickName FROM board WHERE idx = 1;
-- 앞의 닉네임을 자식 테이블에서 가져와서 보여준다.
SELECT (SELECT nickName AS replyCnt from boardReply where idx = 1),nickName 
from board where idx = 1;


-- 부모관점에서 고유번호 1번의 아이디와 현재그렝 달려있는 댓글의 수
SELECT mid,
	(SELECT count(*) AS replyCnt FROM boardReply WHERE boardIdx=1)
	FROM board where idx = 1;
	
-- 부모관점에서 board테이블의 모든 내용과, 현재글에 달려있는 댓글의 개수를 가져오되, 최근글 5개만 출력
SELECT *,
	(SELECT count(*) AS replyCnt FROM boardReply WHERE boardIdx=board.idx)
	FROM board order by idx desc 
	limit 5;

-- 부모관점에서 board테이블의 모든 내용과, 현재글에 달려있는 댓글의 개수를 가져오되, 최근글 5개만 출력
-- 각각의 테이블에 별명을 넣어서 앞의 내용을 변경 시켜보자.
SELECT *,
	(SELECT count(*) AS replyCnt FROM boardReply WHERE boardIdx=b.idx)
	FROM board b
	order by idx desc 
	limit 5;
desc boardReply;
	
SELECT (SELECT count(*) FROM boardReply  WHERE boardIdx=b.idx) FROM board b WHERE idx = 1
